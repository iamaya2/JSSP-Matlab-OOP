function[FeatureValue] = Mirsh175(JSSPInstance)
    InstanceData=JSSPInstance.rawInstanceData;
        for y=1:JSSPInstance.nbMachines %repeat for each operation slot
            subOSCOMBA=0;
            for x=1:JSSPInstance.nbJobs-1 %repeat for each activity on the operation slot
               ...
               if x>1  %testing after the first machine
                    for z=x-1:-1:1 %looking if this machine has not appeared before
                        if InstanceData(z,y,2)==InstanceData(x,y,2)%testing if the machine has appeared before 
                    ...
                    if bN==true %Testing if this machine has not appeared befor
                        for a=x+1:JSSPInstance.nbJobs %Compare to the next machines until the last one
                            if InstanceData(a,y,2)==InstanceData(x,y,2)%testing if the machine has a repetition 
        ...
        meanOPT = OPT/(JSSPInstance.nbMachines*JSSPInstance.nbJobs);
        
        
        FeatureValue=(mean(OSCOMBA)/(JSSPInstance.nbMachines*meanOPT));
